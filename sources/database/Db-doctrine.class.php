...
if (!defined('DB_TYPE'))
	define('DB_TYPE', 'Doctrine');

use Doctrine\Common\ClassLoader;
use Doctrine\DBAL\DriverManager;

require SOURCEDIR . '/database/vendor/autoload.php';

/**
 * SQL database class, implements database class to control mysql functions
 */
class Database_Doctrine extends Database_Abstract
{
	/**
	 * Holds current instance of the class
	 * @var Database_Doctrine
	 */
...
// 	private $_connection = null;
	...
	protected function __construct()
	{
		...
		$this->_classloader = new ClassLoader('Doctrine', SOURCEDIR . '/database/vendor/doctrine');
	...
	{
		global $mysql_set_mode, $db_driver;
	...
	// Connection gone???  This should *never* happen at this point, yet it does :'(
		...
		{
			case 'int':
				$this->_binds[] = (string) (int) $replacement;
				$this->_types[] = \PDO::PARAM_INT;
				return '?';
			break;

			case 'string':
			case 'text':
				$this->_binds[] = $replacement;
				$this->_types[] = \PDO::PARAM_STR;
			...
			case 'array_int':
				if (is_array($replacement))
				{
					...
					$this->_binds[] = $replacement;
					$this->_types[] = \Doctrine\DBAL\Connection::PARAM_INT_ARRAY;
			...
			case 'array_string':
				if (is_array($replacement))
				{
					...
					$this->_binds[] = $replacement;
					$this->_types[] = \Doctrine\DBAL\Connection::PARAM_STR_ARRAY;
			...
			case 'date':
				...
				{
					$this->_binds[] = sprintf('\'%04d-%02d-%02d\'', $date_matches[1], $date_matches[2], $date_matches[3]);
					$this->_types[] = \PDO::PARAM_STR;
			...
			case 'float':
				...
				$this->_binds[] = (string) (float) $replacement;
				$this->_types[] = \PDO::PARAM_STR;
...
// 			break;
	...
	public function quote($db_string, $db_values, $connection = null)
	{
		...
		if (strpos($db_string, '{') !== false)
		{
			...
			$this->_db_callback_connection = $connection === null ? $this->_connection : $connection;
	...
	public function query($identifier, $db_string, $db_values = array(), $connection = null)
	{
		...
		if(empty($db_values['security_override']))
		{
			$this->_binds = array();
			$this->_types = array();
		}
		...
		$this->_result = $this->prepare_results($db_string, $this->_binds, $this->_types);
		$this->_pointer = 0;

		$ret = $this->_connection;

		if ($this->_result === false && empty($db_values['db_error_skip']))
			$this->_result = $this->error($db_string, $connection);
	...
	protected function prepare_results($db_string, $binds, $types)
	{
		try
		{
			$this->_last_query = array($db_string, $binds, $types);
			$statement = $this->_connection->executeQuery($db_string, $binds, $types);
		}
		catch (Exception $e)
		{
			echo "<hr>executeQuery\n" . $db_string . '<pre>';
			print_r($binds);
			print_r($types);
	...
	{
		// Just delegate to Doctrine as usual
		if (isset($this->_result[$this->_pointer]))
			return array_values($this->_result[$this->_pointer++]);
		else
			return null;
	}
	...
	public function free_result($result)
	{
		...
		// I can't find an equivalent at the moment
	}
	...
	{
		$command = substr(trim($this->_last_query[0]), 0, 6);

		if ($command === 'SELECT' || $command === 'DELETE' || $command === 'UPDATE' || $command === 'INSERT')
		{
			return $this->affected_rows;
		}
		else
	 ...
	 */
	public function data_seek($request, $counter = null)
	{
		if ($counter !== null)
		{
			$this->_pointer = (int) $counter;
		}

		return $this->fetch_assoc($request);
	}
...
// 
	...
	public function insert($method = 'replace', $table, $columns, $data, $keys, $disable_trans = false, $connection = null)
	{
		...
		{
			$inserts = array_combine(array_keys($columns), $vals);
			try
			{
				$this->_connection->insert($table, $inserts);
			}
			catch(Exception $e)
			{
				if ($method === 'replace' && !empty($keys))
				{
					$where = array();
					foreach ($keys as $key)
					{
						$where[$key] = $inserts[$key];
					}
					$this->_connection->update($table, $inserts, $where);
				}
	...
	public function fetch_assoc($request, $counter = false)
	{
		// Just delegate to Doctrine as usual
		if (isset($this->_result[$this->_pointer]))
			return $this->_result[$this->_pointer++];
		else
			return null;
	}
